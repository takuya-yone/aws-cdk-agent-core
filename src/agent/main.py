...
def save_invocation_log(
    invocation_id: str, actor_id: str, session_id: str, input: str, output: str
):
...
async def entrypoint(payload: dict):
    ...
    logger.info(
        "Invocation started...",
        extra={"invocation_id": invocation_id, "payload": payload},
    )
    ...
    agentcore_memory_config = AgentCoreMemoryConfig(
        memory_id=memory_settings.memory_id, session_id=session_id, actor_id=actor_id
    )
        ...
        if "event" in msg:
            ...
            if "contentBlockDelta" in msg["event"]:
                response_msg_list.append(
                    msg["event"]["contentBlockDelta"]["delta"].get("text", "")
                )

    # Save the invocation log after processing
    save_invocation_log(
        invocation_id, actor_id, session_id, user_prompt, "".join(response_msg_list)
    )
