...
def save_invocation_log(
    ...
    :param metadata: The metadata containing token usage information
    :type metadata: MetadataModel
...
async def entrypoint(invocation_id: str, payload: InvocationRequestModel):
    ...
    async for msg in stream_messages:
        if "event" in msg:
            ...
            if response.event == EventTypeEnum.metadata:
                metadata = MetadataAttribute.from_dict(
                    event[EventTypeEnum.metadata.value]["usage"]
                )
    ...
    save_invocation_log(invocation_id, payload, metadata, "".join(response_msg_list))
